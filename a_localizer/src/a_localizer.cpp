...
EMCL::EMCL(): Node("emcl") //,engine_(seed_gen_())
{
    // ROSのパラメータの取得(EMCL)
    ...
    sub_odom_  = this -> create_subscription<nav_msgs::msg::Odometry>("/roomba/odometry",rclcpp::QoS(1).reliable(),std::bind(&EMCL::odom_callback, this,std::placeholders::_1));
    ...
    pub_estimated_pose_ = this -> create_publisher<geometry_msgs::msg::PoseStamped>("/estimated_pose",rclcpp::QoS(1));
    pub_particle_cloud_ = this -> create_publisher<geometry_msgs::msg::PoseArray>("/particle_cloud",rclcpp::QoS(1));
}
